...
CREATE TABLE DEMOFILE(
	...
	STORED_FILE_NAME VARCHAR2(300) NOT NULL,
...
CREATE SEQUENCE DEMOFILE_SEQ START WITH 1 INCREMENT BY 1 NOCACHE;

COMMENT ON TABLE [테이블명] IS [COMMENT];
COMMENT ON TABLE EX_TABLE IS '예제 테이블';
COMMENT ON COLUMN [테이블명].[컬럼명] IS '[COMMENT]';

--DEMOFILE TABLE COMMENT 입력 쿼리
COMMENT ON TABLE DEMOFILE IS '첨부파일';
COMMENT ON COLUMN DEMOFILE.FILE_SEQ  IS '첨부파일 번호';
COMMENT ON COLUMN DEMOFILE.DEMOBOARD_SEQ  IS '게시글 번호';
COMMENT ON COLUMN DEMOFILE.ORIGINAL_FILE_NAME  IS '원본 파일 이름';
COMMENT ON COLUMN DEMOFILE.STORED_FILE_NAME  IS 'DB저장 파일 이름';
COMMENT ON COLUMN DEMOFILE.FILE_PATH  IS '파일 업로드 경로';
COMMENT ON COLUMN DEMOFILE.FILE_TYPE  IS '이미지 판단';
COMMENT ON COLUMN DEMOFILE.REGDATE  IS '파일등록 날짜';

--(1)DEMOFILE 새로 추가 쿼리
INSERT INTO DEMOFILE
(FILE_SEQ, DEMOBOARD_SEQ, ORIGINAL_FILE_NAME, STORED_FILE_NAME, FILE_PATH, REGDATE)
VALUES(DEMOFILE_SEQ.NEXTVAL , 1, 'HI', 'DSS', 'C:\eclipse_web_2020_09\workspace_web\20220508_DemoProject',SYSDATE );

--(2)DEMOFILE 첨부파일 삭제 쿼리 
DELETE FROM DEMOFILE WHERE STORED_FILE_NAME = '';

--(3)DEMOFILE 게시물 번호에 해당하는 첨부파일 조회 쿼리
SELECT * 
	FROM DEMOFILE d 
		WHERE DEMOBOARD_SEQ = '';